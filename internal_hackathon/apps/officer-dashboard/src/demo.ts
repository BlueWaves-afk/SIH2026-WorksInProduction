...
export const demoEvents: RiskEvent[] = [
  {
    ...
    contributors: [driver("rainfall_deficit", 20, 20, "Rainfall is 28% below the seasonal normal", "IMD replay"), driver("price_shock", 18, 20, "Cotton modal price is 17% below its seasonal baseline", "Agmarknet replay"), driver("satellite_crop_stress", 11, 15, "Satellite crop signal shows growing vegetation stress", "Sentinel-2 replay")], action_ids: ["action-cotton-shock-1"], model_version: "fdi-v2-demo", evaluated_at: NOW, expires_at: "2026-08-21T05:30:00.000Z", disclaimer: "This is not a credit, loan-default, or insurance score.", context_flags: ["price_feed_fresh"],
  },
  {
    event_id: "evt-demo-ps02-002", farmer_token: "farmer-demo-token-2", village_id: "Nashik / Kalwan", score: 58, band: "amber", confidence: .73,
    contributors: [driver("rainfall_deficit", 15, 20, "Rainfall is 19% below the seasonal normal", "IMD replay"), driver("price_shock", 12, 20, "Soybean price is below the three-year seasonal baseline", "Agmarknet replay"), driver("irrigation_gap", 7, 10, "Rain-fed field has limited irrigation fallback", "Farmer profile")], action_ids: ["action-soybean-check-1"], model_version: "fdi-v2-demo", evaluated_at: NOW, expires_at: "2026-08-21T05:30:00.000Z", disclaimer: "This is not a credit, loan-default, or insurance score.", context_flags: ["weather_feed_fresh"],
  },
  {
    event_id: "evt-demo-ps02-003", farmer_token: "farmer-demo-token-3", village_id: "Nashik / Peint", score: 31, band: "green", confidence: .91,
    contributors: [driver("rainfall_deficit", 8, 20, "Rainfall is close to the seasonal normal", "IMD replay"), driver("price_shock", 5, 20, "Market price is within the expected seasonal range", "Agmarknet replay")], action_ids: [], model_version: "fdi-v2-demo", evaluated_at: NOW, expires_at: "2026-08-21T05:30:00.000Z", disclaimer: "This is not a credit, loan-default, or insurance score.", context_flags: [],
  },
];
...
export const demoBrief: CopilotBrief = {
  ...
  scheme_matches: [{ scheme: "PMFBY", why: "The farmer may be eligible for crop-loss support; an officer will confirm.", verified: false, citations: [{ source_doc: "PMFBY operational guidelines", chunk_id: "pmfby-demo-14", quote: "Coverage and claim decisions follow the applicable notified area and season." }] }],
  ...
  citations: [{ source_doc: "PMFBY operational guidelines", chunk_id: "pmfby-demo-14", quote: "Coverage and claim decisions follow the applicable notified area and season." }],
  model_version: "template-m7-demo",
};
