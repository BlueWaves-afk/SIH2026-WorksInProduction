...
Every farmer tool today is *reactive*: the farmer must open an app and ask (chatbot), photograph a diseased leaf (Plantix/AgroStar), or check prices (eNAM). AgriStack + RBI's lending rail decide *who to lend to*. **Nobody decides who is under stress and needs a human to reach out — before the farmer asks.** KisanSetu fills exactly that gap: a transparent, rules-based support-priority radar that runs per village, explains itself, speaks the farmer's language by voice, and hands a ranked, reason-coded queue to an agriculture officer. The winning line:

> *"KisanSetu doesn't predict who will default. It notices who may need support early, explains why, and connects them to a person."*
...
| `phone` | string (encrypted) | SMS/voice/callback delivery | Onboarding | delivery |
| `consent_flags` | object | Legal basis for storage/contact/analytics | Toggle screen | — |
...
FarmerProfile { farmer_token, village_id, locale, crop, sowing_date,
                irrigation_type, area_band, phone_enc, consent_flags }
Observation   { source, observed_at, village_id|plot_grid, metric,
                value, unit, quality, ttl }
...
AlertCase     { event_id, recipient_role, channel, sent_at, ack_at, status,
...
**Positioning:** KisanSetu is the **human-outreach layer that sits on top of AgriStack** — the same public signals that power lending decisions are used to decide *who's under stress and needs a person to call*, the piece the ecosystem is missing. This plugs into the government's stated direction (DPI, "next UPI," Bharat-VISTAAR) instead of competing with it.
...
9. Close on the line: *"KisanSetu doesn't predict who will default. It notices who may need support early, explains why, and connects them to a person."*
...
**Whitespace KisanSetu owns:** proactive, explainable, population-scale distress triage routed to a human officer — the "anticipatory support via early warning" the distress literature explicitly calls for and that no shipping product provides.
